...
namespace BibleNote.Tests
{
    ...
    public class ParagraphParserTests : TestsBase.TestsBase
    {
        ...
        private TestResult CheckVerses(string input, string expectedOutput, Action<IDocumentParseContextEditor> initDocParseContext, string[] notFoundVerses, params string[] verses)
        {
            ...
            Assert.AreEqual(verses.Length, result.VerseEntries.Count, string.Format("Verses length is not the same. Expected: {0}. Found: {1}", verses.Length, result.VerseEntries.Count));            
            var versePointers = result.VerseEntries.Select(ve => ve.VersePointer);
            foreach (var verse in verses)
                Assert.IsTrue(versePointers.Contains(this.versePointerFactory.CreateVersePointer(verse)), string.Format("Can not find the verse: '{0}'", verse));            
...
lang=ru>&nbsp;Тим&nbsp;</span><span style='color:#444444' lang=en-US>2:2). </span>";
    ...
    ";
        ...
        public void Test52()
        {
            var input = "Слово, переведённое в 2Кор 9:7 и 8 как «расположение», имеет следующие значения: выбирать, избирать, предпочитать одно перед другим, что говорит об обдуманном, взвешенном решении, принятом в своей жизни. " +
                        "В некоторых церквах рекомендуют придерживаться следующего принципа: 10% – на поддержку церкви и заботу о нуждающихся, 20% – на сбережения, 80% – на всё остальное.";
            CheckVerses(input, null, null, "2Кор 9:7");
        }
